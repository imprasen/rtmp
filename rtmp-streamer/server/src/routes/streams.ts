...
streamsRouter.get("/", async (req: Request, res: Response) => {
  ...
  const result = streams.map((s) => {
    ...
    return {
      ...
      obs_server: `rtmp://${rtmpHost}:1935/ingest`,
      obs_key: s.stream_key,
      domain_rtmp: `rtmp://${rtmpHost}:1935/ingest/${s.stream_key}`,
...
streamsRouter.get("/:idOrKey", async (req: Request, res: Response) => {
  ...
  return res.json({
    ...
    domain_rtmp: `rtmp://${rtmpHost}:1935/ingest/${stream.stream_key}`,
...
streamsRouter.post("/", requireAuth, (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      obs_server: `rtmp://${rtmpHost}:1935/ingest`,
      obs_key: key,
      rtmp_url: `rtmp://${publicHost}:1935/ingest/${key}`,
      domain_rtmp: `rtmp://${rtmpHost}:1935/ingest/${key}`,
    });
  } catch (err: any) {
